...
if target_resid == 0 
    if printEvery > 0 && printEvery < Inf && verbose
...
%             atom_new    = atom_new - (atom_new'*A_T(:,j))*A_T(:,j);
    ...
    normR   = norm(r);
    % -- Print some info --
    PRINT   = ( ~mod( kk, printEvery ) || kk == k ) && verbose;
    if printEvery > 0 && printEvery < Inf && (normR < target_resid ) && verbose
...
if (target_resid) && ( normR >= target_resid ) && verbose
